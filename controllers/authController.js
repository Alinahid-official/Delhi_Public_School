...
//------------ User Model ------------//
const studentUser = require('../src/models/studentUser');
...
exports.registerHandle = (req, res) => {
    ...
    if (errors.length > 0) {
        ...
        //------------ Validation passed ------------//
        studentUser.findOne({ email: email }).then(user => {
            if (user) {
                //------------ User already exists ------------//
    ...
    if (errors.length > 0) {
        ...
        studentUser.findOne({ email: email }).then(user => {
            if (!user) {
                ...
                studentUser.updateOne({ resetLink: token }, (err, success) => {
                    if (err) {
    ...
    if (token) {
        jwt.verify(token, JWT_RESET_KEY, (err, decodedToken) => {
            if (err) {
                ...
                studentUser.findById(_id, (err, user) => {
                    if (err) {
                        req.flash(
                            'error_msg',
